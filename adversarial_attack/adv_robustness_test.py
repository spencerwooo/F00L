...
def init_models(model_name, model_path):
  model = utils.load_trained_model(
    model_name=model_name, model_path=model_path, class_num=len(CLASS_NAMES)
  )
...
def main():
  ...
  print("\n[TASK 1/3] Validate original adversaries:")
  utils.validate(fmodel, dataset_loader, dataset_size, batch_size=BATCH_SIZE, advs=advs)
